  ...
  return <Layout>
      <div className="flex-col md:flex-row md:justify-between text-sm, font-semibold w-full ">
        <div className="flex items-center space-x-4">
          <Link to="/products" className="flex items-center space-x-2 text-muted-foreground hover:text-primary transition-colors">
            <ArrowLeft className="h-4 w-4" />
            <span>Continue Shopping</span>
          </Link>
          <h1 className="font-bold text-foreground text-2xl">Shopping Cart</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-4">
            {cartItems.map(item => <div key={item.id} className="bg-card border border-border rounded-lg p-4">
                <div className="flex items-start space-x-4">
                  <div className="relative">
                    <img src={item.image} alt={item.name} className="w-24 h-24 object-cover rounded-md bg-accent/30" />
                    {item.onSale && <span className="absolute -top-2 -right-2 bg-destructive text-destructive-foreground text-xs px-2 py-1 rounded">
                        Sale
                      </span>}
                  </div>
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center space-x-4 mt-3">
                      <div className="flex items-center space-x-2 border border-border rounded-md">
                        <button onClick={() => updateQuantity(item.id, -1)} className="p-2 hover:bg-accent rounded-l-md transition-colors" disabled={item.quantity <= 1}>
                          <Minus className="h-4 w-4" />
                        </button>
                        <span className="w-12 text-center font-medium">{item.quantity}</span>
                        <button onClick={() => updateQuantity(item.id, 1)} className="p-2 hover:bg-accent rounded-r-md transition-colors">
                          <Plus className="h-4 w-4" />
                        </button>
                      </div>
                      
                      <button onClick={() => saveForLater(item.id)} className="text-sm text-primary hover:text-primary/80 transition-colors">
                        Save for Later
                      </button>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-card border border-border rounded-lg p-4">
              ...
              <div className="flex space-x-2">
                <input type="text" placeholder="Enter code" value={promoCode} onChange={e => setPromoCode(e.target.value)} className="flex-1 px-3 py-2 border border-input rounded-md focus:outline-none focus:ring-2 focus:ring-ring text-sm" />
                <button onClick={applyPromoCode} className="px-4 py-2 bg-primary text-primary-foreground rounded-md hover:bg-primary/90 transition-colors text-sm">
                  Apply
                </button>
              </div>
              {promoCode === 'SAVE10' && <p className="text-sm text-green-600 mt-2">✓ Code applied: $10 off</p>}
            </div>
            ...
            <div className="bg-card border border-border rounded-lg p-4">
              ...
              <div className="flex items-center justify-between">
                ...
                <label className="flex items-center">
                  <input type="checkbox" checked={useRewardPoints} onChange={e => setUseRewardPoints(e.target.checked)} className="rounded border-border" />
            ...
            <div className="bg-card border border-border rounded-lg p-6">
              ...
              <div className="space-y-3">
                ...
                {promoDiscount > 0 && <div className="flex justify-between text-green-600">
                    <span>Promo Code Discount</span>
                    <span>-${promoDiscount.toFixed(2)}</span>
                  </div>}
                
                {rewardDiscount > 0 && <div className="flex justify-between text-green-600">
